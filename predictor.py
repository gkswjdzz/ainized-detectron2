...
class VisualizationDemo(object):
    def __init__(self, cfg, debug, instance_mode=ColorMode.IMAGE, parallel=False):
        """
        Args:
            cfg (CfgNode):
            ...
            if "instances" in predictions:
                ...
                if not debug :
                    ...
                    for i, _ in enumerate(labels) :
                        ...
                        tmp['box'] = {}
                        tmp['box']['left-up'] = [boxes[i][0].item(), boxes[i][1].item()]
                        tmp['box']['right-down'] = [boxes[i][2].item(), boxes[i][3].item()]
                        tmp['polygons'] = {}

                        if masks is not None:
                            for idx, segment in enumerate(masks[i].polygons):
                                tmp['polygons'][idx] = segment.reshape(-1, 2).tolist()
